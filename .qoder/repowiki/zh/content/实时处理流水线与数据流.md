...
- 改进了数据传输和帧缓冲机制
- 更新了实时处理流水线的端到端数据流时序
- **新增 Web 实时显示旁路**：WsFrameSink（IResultSink 扇出）+ 二进制 wire 协议 + 浏览器前端（web/），由 radar_web_demo 定时回放驱动
- 修正了 DSP 处理流水线的实际算子顺序（Parse → RangeFFT → PhaseUnwrap → ClutterRemoval → DopplerFFT → CA-CFAR → AngleFFT）
...
### DSP处理流水线
实际算子顺序（见 `src/tools/radar_dsp_demo.cpp` / `radar_web_demo.cpp` 的组装顺序）：

- ParseStage（I/Q 反交织解析）→ RangeFftStage（距离 FFT，Hann 窗 + 去直流）
- PhaseUnwrapStage（相位解缠/生命体征）**有意放在杂波抑制之前**：相位取自未滤波的 rangeCube
- ClutterRemovalStage（MTI 杂波抑制）→ DopplerFftStage（多普勒 FFT）
- CfarStage（CA-CFAR 检测）→ AngleFftStage（角度 FFT，补充检测的角度维）

**新增** 完整的DSP处理流水线，提供从原始数据到最终结果的完整处理链。

```mermaid
flowchart LR
Input["原始字节帧"] --> Parse["ParseStage<br/>I/Q 反交织"]
Parse --> RangeFFT["RangeFftStage<br/>距离FFT"]
RangeFFT --> Phase["PhaseUnwrapStage<br/>相位解缠（未滤波cube）"]
Phase --> Clutter["ClutterRemovalStage<br/>杂波抑制"]
Clutter --> Doppler["DopplerFftStage<br/>多普勒FFT"]
Doppler --> CFAR["CfarStage<br/>CA-CFAR检测"]
CFAR --> AngleFFT["AngleFftStage<br/>角度FFT"]
AngleFFT --> Sinks["IResultSink 扇出<br/>(CSV / WebSocket)"]
```
...
### Web 实时显示旁路（WsFrameSink 扇出）
Pipeline 处理完每帧后扇出给所有 IResultSink；新增的 WsFrameSink 把帧编码为二进制 wire 包推送到浏览器，形成与 CSV/落盘并列的**显示旁路**：

- **协议**（`src/web/WireProtocol.h`）：每帧一条 binary 消息（~2 KB：头部 36 B + 原始 ADC I/Q int16×2 + 距离谱 dB float32），客户端接入时另发一次 meta JSON（nWave/nBins/rangeIdxToMeters/fps/λ）；所有字段小端，前端 DataView/TypedArray 零拷贝解码。
- **线程模型**：consume() 在 DSP worker 上只做编码 + `SpscRing::try_push`（容量 64，满则丢帧计数）；专用发送线程 pop 后向所有客户端 sendBinary——网络永不反压 DSP。
- **驱动入口**：`radar_web_demo <bin> [port] [--loop]` 按 `framePeriodicityMs`（数据集 50 ms = 20 fps）`sleep_until` 定时提交，是首个走完整 `Pipeline::addSink` 路径的实数据工具。
- **前端**（`web/`，零构建工具链）：uPlot 四图（原始 ADC、距离谱+跟踪 bin、相位/位移 60 s 滚动、呼吸带通波形），控制栏含连接/断开、暂停/继续、清空；断线 1 s 自动重连，手动断开不重连。

```mermaid
sequenceDiagram
participant W as "Pipeline worker"
participant S as "WsFrameSink.consume()"
participant Q as "SpscRing(64)"
participant T as "发送线程"
participant B as "浏览器 app.js"
W->>S : "扇出 FrameContext"
S->>S : "encodeFramePacket()<br/>(无效/形状漂移 => 跳过)"
S->>Q : "try_push（满则丢帧+计数）"
S-->>W : "立即返回（零阻塞）"
T->>Q : "pop"
T->>B : "sendBinary（逐客户端）"
B->>B : "DataView 解码 -> 环形缓冲"
B->>B : "requestAnimationFrame 统一重绘"
```

扩展新算法的实时显示（协议加字段、前端加图表、RBM/呼吸案例）见《实时显示扩展开发指南》。
