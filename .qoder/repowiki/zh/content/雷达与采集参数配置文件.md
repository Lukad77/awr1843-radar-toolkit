...
## 更新摘要
**变更内容**
- 新增 RadarConfig 集中式配置管理模块，作为所有采集和处理参数的单一真实来源
- 统一配置推导和验证机制，取代之前分散在 RadarParams、AWR1843Controller::ConfigParams 中的配置表示
- 增强配置一致性检查和错误处理
- 保持向后兼容性，旧接口仍然可用
...
- 说明新的 RadarConfig 集中式配置管理如何统一处理配置推导，以及 AWR1843Controller 如何读取并下发这些配置，并从 profileCfg/frameCfg/channelCfg 推导距离分辨率、多普勒分辨率、最大距离/速度等雷达参数。
...
## 核心组件
- **RadarConfig**：新的集中式配置管理核心，提供统一的配置推导和验证机制，取代之前分散的配置表示。
...
- ParseStage：基于 RadarConfig 的新式解析阶段，支持更高效的内存管理和并行处理。
...
- [ParseStage.h:23-48](file://src/pipeline/ParseStage.h#L23-L48)
- [cf.json:1-10](file://awr1843_dca1000_read/cf.json#L1-L10)

## 架构总览
下图展示了从配置文件到参数推导与设备控制的端到端流程，突出显示新的 RadarConfig 集中式配置管理：

```mermaid
sequenceDiagram
participant Main as "主程序"
participant RC as "RadarConfig<br/>集中式配置管理"
...
图表来源
- [RadarConfig.h:51-56](file://src/core/RadarConfig.h#L51-L56)
...
**新增** RadarConfig 是新的配置管理核心，提供了统一的配置推导和验证机制：

#### 主要字段分类
- **数据格式字段**：numAdcBits、isReal、numRxAnt、numTxAnt、numAdcSamples、rxIdx
- **帧配置字段**：chirpStartIdx、chirpEndIdx、numLoops、numFrames、framePeriodicityMs、triggerSelect、triggerDelay  
- **波形配置字段**：startFreqGHz、idleTimeUs、rampEndTimeUs、freqSlopeMHzPerUs、digOutSampleRateKsps
- **派生字段**：numChirpsPerFrame、numDopplerBins、numRangeBins、bytesPerSample、bytesPerFrame、rangeResolutionMeters、dopplerResolutionMps、maxRange、maxVelocity

#### 核心方法
- `derive()`：计算所有派生字段，包括帧大小、分辨率、最大范围/速度等
- `validate()`：验证配置一致性，返回错误信息

```mermaid
flowchart TD
Start(["调用derive()"]) --> SetBytes["设置bytesPerSample<br/>(real=2, complex=4)"]
SetBytes --> CalcChirps["计算numChirpsPerFrame<br/>(chirpEnd-chirpStart+1)*numLoops"]
CalcChirps --> CalcBins["计算numDopplerBins<br/>和numRangeBins"]
CalcBins --> CalcFrameSize["计算bytesPerFrame"]
CalcFrameSize --> CalcResolutions["计算距离/多普勒分辨率"]
CalcResolutions --> CalcMaxValues["计算最大范围/速度"]
CalcMaxValues --> End(["完成推导"])
```

图表来源
- [RadarConfig.cpp:16-56](file://src/core/RadarConfig.cpp#L16-L56)

章节来源
- [RadarConfig.h:14-56](file://src/core/RadarConfig.h#L14-L56)
- [RadarConfig.cpp:16-76](file://src/core/RadarConfig.cpp#L16-L76)
- [test_core.cpp:123-162](file://src/tests/test_core.cpp#L123-L162)
...
### 新式 ParseStage 集成
**新增** ParseStage 是基于 RadarConfig 的新式解析阶段，提供更高效的内存管理和并行处理能力：

- 直接接受 RadarConfig 对象作为构造参数
- 支持单天线和多天线解析模式
- 使用 BufferPool 进行内存池管理，减少分配开销
- 保持与旧版 DataParser 的输出格式兼容性

章节来源
- [ParseStage.h:23-48](file://src/pipeline/ParseStage.h#L23-L48)
- [ParseStage.cpp:1-26](file://src/pipeline/ParseStage.cpp#L1-26)
...
- **新增** RadarConfig 作为独立的配置管理模块，被 ParseStage 等新组件使用。
...
class ParseStage {
+ParseStage(cfg, parseAllRx, pool)
+process(ctx)
+parse(raw, fb)
+expectedBytes()
}
...
- **新增** RadarConfig 的 derive() 方法只计算一次派生字段，validate() 提供快速配置验证，避免重复计算。
- **新增** ParseStage 使用 BufferPool 进行内存池管理，减少频繁的内存分配和释放开销。
...
- **新增** RadarConfig 验证失败：检查 derive() 是否正确调用，validate() 返回的错误信息会详细说明配置不一致的原因。
- **新增** ParseStage 解析失败：确认 expectedBytes() 与实际数据大小匹配，检查 RadarConfig 的 bytesPerFrame 计算是否正确。
...
- [ParseStage.h:33-41](file://src/pipeline/ParseStage.h#L33-L41)

## 结论
- 1T1R.cfg 与 awr1843.cfg 通过 mmWave CLI 命令定义雷达波形、通道与帧结构；AWR1843Controller 能自动解析并推导关键物理参数，便于上层算法直接使用。
- **新增** RadarConfig 作为集中式配置管理核心，提供了统一的配置推导和验证机制，取代了之前分散的配置表示，提高了配置的一致性和可维护性。
- cf.json 提供 DCA1000 的采集模式与数据包参数，与 lvdsStreamCfg 协同确保正确的数据流格式与网络传输节奏。
- **新增** ParseStage 等新组件基于 RadarConfig 构建，提供更好的性能和可扩展性。
- 建议在实际部署中严格对齐 RadarConfig 与配置文件，确保帧大小与解析契约一致，避免数据错位或解析失败。

[本节为总结性内容，不涉及具体文件分析]

## 附录
- 帧字节数计算公式（参考注释）：BYTES_PER_FRAME = numRX × numChirpsPerFrame × numADCSamples × 4（每采样点为 2×int16 的 I/Q）。
- 网络地址约定：上位机 IP 192.168.33.30，DCA1000 IP 192.168.33.180。
- **新增** RadarConfig 派生公式：
  - numChirpsPerFrame = (chirpEndIdx - chirpStartIdx + 1) × numLoops
  - numDopplerBins = numChirpsPerFrame / numTxAnt
  - numRangeBins = ceil_pow2(numAdcSamples)
  - rangeResolutionMeters = (c × digOutSampleRateKsps × 1e3) / (2 × freqSlopeMHzPerUs × 1e12 × numAdcSamples)
  - dopplerResolutionMps = c / (2 × startFreqGHz × 1e9 × (idleTimeUs + rampEndTimeUs) × 1e-6 × numDopplerBins × numTxAnt)
