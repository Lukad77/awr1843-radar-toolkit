...
## 更新摘要
**所做更改**
- 新增 Web 实时显示子系统：`src/web/`（WsFrameSink + 二进制 wire 协议）、`web/`（浏览器单页前端）、`src/tools/radar_web_demo.cpp`（回放驱动入口）、`third_party/ixwebsocket/`（vendored 可选依赖）
- "零第三方依赖"表述更新为"核心链路零第三方依赖；Web 显示为可选 vendored 依赖，随 RADAR_BUILD_WEB 开关整体启停"
...
### Web 实时显示子系统（src/web + web/，新增）
- 组成：WsFrameSink（IResultSink 实现，挂在新架构 Pipeline 扇出上）→ 二进制 wire 协议（WireProtocol.h）→ ws://localhost:8765 → 浏览器单页（web/index.html + app.js + vendored uPlot，零构建工具链）。
- 显示内容：原始 ADC 波形、距离谱（含跟踪 bin 标记）、解缠相位/位移 60 s 滚动、呼吸带通波形与呼吸率；页面控制栏支持连接/断开、暂停/继续、清空。
- 入口：`radar_web_demo <bin> [port] [--loop]`（离线回放，20 fps 定时提交）；构建由 `RADAR_BUILD_WEB` 选项与 vendored IXWebSocket（third_party/，关闭 TLS/zlib）控制，可整体裁剪。
- 关键约定：显示旁路允许丢帧、绝不反压 DSP worker（consume 只编码+try_push，发送在专用线程）。扩展指南见《实时显示扩展开发指南》。
