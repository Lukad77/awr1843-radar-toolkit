...
// (no hardware): Parse -> RangeFFT -> DopplerFFT -> CA-CFAR -> AngleFFT ->
// PhaseUnwrap, with per-stage timing and a respiration-phase CSV.
//
...
int main(int argc, char** argv) {
    ...
    auto phaseStage = std::make_shared<PhaseUnwrapStage>(cfg, PhaseUnwrapParams{});
    ...
    while (in.read(reinterpret_cast<char*>(raw->data()),
        ...
        if (!ctx.valid) ++invalid;
        if (ctx.detections) {
            totalDets += ctx.detections->size();
            for (const auto& d : *ctx.detections)
                if (d.snrDb > strongestSnr) { strongestSnr = d.snrDb; strongest = d; }
        }
        ++frames;
        if (frames % 200 == 0) {
            std::cout << "frame " << frames << ": dets="
                      << (ctx.detections ? ctx.detections->size() : 0)
                      << ", phase=" << ctx.unwrappedPhaseRad << " rad, disp="
                      << ctx.displacementMm << " mm" << std::endl;
        }
        if (maxFrames > 0 && frames >= static_cast<std::uint64_t>(maxFrames)) break;
    }
    phaseSink.flush();

    // ---- 汇总 ----
    std::cout << "------------------------------------------" << std::endl;
    std::cout << "处理帧数: " << frames << " (invalid " << invalid << ")" << std::endl;
    if (frames == 0) return 1;
    double totalMs = 0.0;
    const char* names[] = {"Parse", "RangeFFT", "DopplerFFT", "CA-CFAR", "AngleFFT",
                           "PhaseUnwrap"};
    for (std::size_t i = 0; i < stages.size(); ++i) {
    ...
    std::cout << "平均检测数/帧: " << static_cast<double>(totalDets) / frames << std::endl;
    std::cout << "最强目标: range=" << strongest.rangeM << " m, v="
              << strongest.velocityMps << " m/s, angle=" << strongest.angleDeg
              << " deg, SNR=" << strongestSnr << " dB (rangeBin "
              << strongest.rangeBin << ")" << std::endl;
    std::cout << "呼吸相位跟踪 bin: " << phaseStage->targetBin() << " (≈"
              << phaseStage->targetBin() * cfg.rangeIdxToMeters << " m)" << std::endl;
