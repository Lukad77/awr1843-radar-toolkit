...
void RadarConfig::derive() {
    ...
    if (numChirpsPerFrame < 0) numChirpsPerFrame = 0;
    ...
    bytesPerFrame =
        static_cast<long>(numChirpsPerFrame) * numRxAnt * numAdcSamples * bytesPerSample;
    ...
    if (freqSlopeMHzPerUs != 0.f && numAdcSamples > 0) {
        rangeResolutionMeters = static_cast<float>(
            (c * digOutSampleRateKsps * 1e3) / (2.0 * freqSlopeMHzPerUs * 1e12 * numAdcSamples));
    }
    if (freqSlopeMHzPerUs != 0.f && numRangeBins > 0) {
        rangeIdxToMeters = static_cast<float>(
            (c * digOutSampleRateKsps * 1e3) / (2.0 * freqSlopeMHzPerUs * 1e12 * numRangeBins));
    }
...
bool RadarConfig::validate(std::string& err) const {
    ...
    if (numAdcSamples <= 0 || (numAdcSamples % 2) != 0) fail("numAdcSamples must be positive and even");
    if (numRxAnt <= 0) fail("numRxAnt must be > 0");
    if (numTxAnt <= 0) fail("numTxAnt must be > 0");
    if (rxIdx < 0 || rxIdx >= numRxAnt) fail("rxIdx out of range [0, numRxAnt)");
