...
static void test_fft_basics() {
    ...
    for (std::size_t n = 0; n < N; ++n)
        in[n] = {static_cast<float>(urand() * 2 - 1), static_cast<float>(urand() * 2 - 1)};
    fft = in;
    ...
    bool sym = true;
    for (int i = 0; i < 8; ++i)
        if (std::abs(w[i] - w[7 - i]) > 1e-6f) sym = false;
    CHECK(sym);
}
...
static void test_doppler_fft() {
    ...
    for (std::size_t i = 1; i < ctx.rdMap->size(); ++i)
        if ((*ctx.rdMap)[i] > (*ctx.rdMap)[argmax]) argmax = i;
...
static void test_cfar() {
    ...
    for (auto& v : *rd) v = static_cast<float>(-std::log(1.0 - urand()));
    ...
    auto found = [&](int d, int b) {
        for (const auto& det : *ctx.detections)
            if (det.dopplerBin == d && det.rangeBin == b) return true;
        return false;
    };
    CHECK(found(1, 50));
    CHECK(found(2, 200));
...
static void test_phase_unwrap() {
    ...
    for (int t = 0; t < M; ++t) {
        ...
        for (std::size_t c = 0; c < 4; ++c) {
            for (std::size_t b = 0; b < 16; ++b) cube->at(c, 0, b) = {0.01f, 0.f};
    ...
    double maxErr = 0.0;
    for (int t = 0; t < M; ++t)
        maxErr = std::max(maxErr, std::abs((out[t] - out[0]) - (truth[t] - truth[0])));
    CHECK(maxErr < 1e-3);
...
static std::shared_ptr<std::vector<std::uint8_t>> makeRawFrame(const RadarConfig& cfg,
                                                               int targetBin) {
...
public:
    ...
    void consume(const FrameContext& ctx) override {
        ...
        if (!ctx.valid || !ctx.parsed || !ctx.rangeCube || !ctx.dopplerCube ||
            ...
            bool hit = false;
            for (const auto& det : *ctx.detections)
                if (std::abs(det.rangeBin - targetBin_) <= 1) hit = true;
            if (!hit) ++missedTarget;
        }
        if (std::isnan(ctx.unwrappedPhaseRad)) phaseAlways = false;
    }
...
static void test_full_chain_pipeline() {
    ...
    for (std::uint64_t i = 0; i < sink->seqs.size(); ++i)
        if (sink->seqs[i] != i) ordered = false;
