// test_dsp.cpp — Phase 4 测试：radix-2 FFT 正确性（对拍朴素 DFT）、
// RangeFFT / DopplerFFT 峰值定位、CA-CFAR 检测 + Pfa 行为、
// AngleFFT 导向矢量恢复、慢时间相位解缠（含真实 ±pi 缠绕），
// 以及全 stage 链在回压下穿过 Pipeline。无硬件；信号全部合成。
...
// 确定性 LCG（跨平台一致）用于噪声合成。
static std::uint32_t g_rng = 0x12345678u;
static double urand() { // 均匀分布 (0, 1)
...
static void test_fft_basics() {
  // 单音 e^{j*2pi*3n/16} => 能量全在 bin 3，|X[3]| == N。
  FftPlan p16(16);
  ...
  CHECK(maxErr / maxMag < 1e-3f);                     // 与朴素 DFT 一致
...
static void test_range_fft() {
  ...
  // 零填充路径：12 采样 -> 16 个 range bin；3/12 处单音落在 bin 4。
...
static void test_doppler_fft() {
  ...
  // 目标在 range bin 2，逐 chirp 旋转 e^{j*2pi*2c/8} => 未 shift 的
  // 多普勒 bin 2 => shifted 索引 6（= 2 + C/2）。
  ...
  // rdMap 全局最大值位于 (doppler 6, range 2)。
  std::size_t argmax = 0;
...
static void test_cfar() {
  ...
  CHECK(found(1, 50));
  CHECK(found(2, 200));
  CHECK(found(3, 0)); // 截断窗在边缘仍能检出
  CHECK(found(3, 255));
  // Pfa=1e-4 × 1024 个单元 => 期望虚警 ~0.1；留余量。
...
static void test_angle_fft() {
  ...
  steer(2, 3, 20.0);  // +20° 目标
  steer(1, 5, -35.0); // -35° 目标
  steer(3, 6, 0.0);   // 法向（boresight）
...
static void test_phase_unwrap() {
  ...
  cfg.chirpEndIdx = 0;
  cfg.numLoops = 4;
  cfg.startFreqGHz = 77.f; // lambdaM ~ 3.896 mm，用于位移换算
  cfg.derive();
  CHECK(cfg.lambdaM > 0.f);

  // phi(t) = 3*sin(2pi t/40)：幅度 3 rad > pi，atan2 必然缠绕；
  // 帧间步长最大 ~0.47 rad < pi，解缠问题良定。
  ...
  // 位移公式：delta_d = lambda * delta_phi / (4*pi)。
  ...
  CHECK(ctx.phaseTrackBin == 7);                     // 质量位：bin
  CHECK(std::abs(ctx.phaseTrackAmp - 10.f) < 0.01f); // 质量位：原始幅度
}

// 辅助函数：在 `bin` 处放单个复数值（所有 chirp）的帧，其余 bin 为微小底噪。
static std::shared_ptr<FrameBuffer>
binFrame(int nBins, int bin, std::complex<float> v, int bin2 = -1,
...
// 先复现平台机制，再由 Kasa DC 补偿消除：
// v = S + a*e^{j*phi(t)}，|S| ~ 5a 时 arg(v) 被钉在 arg(S) 附近。
static void test_phase_dc_compensation() {
  ...
  // A) 补偿关闭 => 平台：恢复的峰峰值被压缩到 ~a/|S| 量级。
  {
    ...
    CHECK((hi - lo) < 0.35f * truthP2p); // 平台已复现
  }
...
static void test_phase_bridge_on_switch() {
  ...
  PhaseUnwrapParams p; // 自动 + followPeak
  p.followPeak = true;
  p.switchHoldFrames = 3;
  p.switchRatio = 1.2f;
  p.dcCompensation = false; // 隔离出桥接行为单独验证
  PhaseUnwrapStage stage(cfg, p);
...
static void test_phase_switch_hysteresis() {
  ...
  CHECK(stage.targetBin() == 4); // 第 0 帧锁定，从不乒乓
}

// 退化圆拟合（真正静止的目标：零张角窗）必须保持信号不补偿
// —— 输出稳定、无 NaN、绝不产生伪圆心。
static void test_phase_dc_degenerate() {
  ...
  bool stable = true;
  for (int t = 0; t < 100; ++t) { // 窗填满；拟合必须保持退化判定
    FrameContext ctx;
...
static void test_clutter_removal() {
  ...
  for (int t = 0; t < M; ++t) {
    ...
    for (std::size_t c = 0; c < 8; ++c)
      for (std::size_t r = 0; r < 2; ++r) {
        // 静态杂波从第 0 帧起被消除（首帧播种杂波图）。
        if (std::abs(cube->at(c, r, 2)) > 1e-3f)
          staticGone = false;
        // 多普勒旋转目标不受影响（其 chirp 均值为零）。
  ...
  // 形状变化干净地重置杂波图（不崩溃，静态再次被消除）。
  auto cube2 = std::make_shared<FrameBuffer>(FrameShape{4, 1, 16});
  for (std::size_t c = 0; c < 4; ++c)
    cube2->at(c, 0, 7) = {5.f, -5.f};
  FrameContext ctx2;
  ctx2.rangeCube = cube2;
  CHECK(stage.process(ctx2));
  bool resetOk = true;
  for (std::size_t c = 0; c < 4; ++c)
    if (std::abs(cube2->at(c, 0, 7)) > 1e-3f)
      resetOk = false;
  CHECK(resetOk);
}

// ---------------------------------------------------------------------------
// 全链穿过 Pipeline（回压 + 顺序 + 各阶段产物）
// ---------------------------------------------------------------------------
  ...
  for (int c = 0; c < C; ++c) {
    for (int r = 0; r < R; ++r) {
      ...
      for (int s = 0; s < S; ++s) {
        ...
        const int ni = static_cast<int>(urand() * 40) - 20; // 底噪
...
static void test_full_chain_pipeline() {
  ...
  Pipeline p(/*inputCapacity=*/4); // 小环 => 真实回压
  ...
  for (std::uint64_t i = 0; i < sink->seqs.size(); ++i)
    if (sink->seqs[i] != i)
      ordered = false;
  CHECK(ordered);                 // 严格保序
  CHECK(sink->allProducts);       // 每个 stage 都产出了制品
  CHECK(sink->missedTarget == 0); // CFAR 每帧都找到注入目标
  CHECK(sink->phaseAlways);       // 每帧都有相位轨迹
}
