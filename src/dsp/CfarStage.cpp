...
bool CfarStage::process(FrameContext& ctx) {
    if (!ctx.valid || !ctx.rdMap) return true;

    const int D = cfg_.numDopplerBins;
    const int B = cfg_.numRangeBins;
    if (D <= 0 || B <= 0 ||
        ctx.rdMap->size() != static_cast<std::size_t>(D) * static_cast<std::size_t>(B)) {
    ...
    for (int d = 0; d < D && static_cast<int>(dets->size()) < p_.maxDetections; ++d) {
        ...
        for (int cut = 0; cut < B; ++cut) {
            ...
            dets->push_back(det);
            if (static_cast<int>(dets->size()) >= p_.maxDetections) break;
        }
    }
